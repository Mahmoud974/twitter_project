...
const ProfilOther = () => {
  ...
  const profilUser = useSelector((profil:ExtendedUser)=> profil.profilUser.otherUsers)
...
{
  profilUser.filter((ok:{id:number})=>Number(id) === ok.id).map((profil:ExtendedUser, index: number) =>
  ...
  <div key={index} className=" mt-4 absolute left-20 ">
        ...
        <img src={profil.image.webp}  alt="profil photo" className="w-28 h-28 border-3 -mt-12 ml-6 rounded-full border-4 relative border-gray-900"/>
